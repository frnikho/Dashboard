...
export default class WidgetNext5DaysForecast extends Widget {
    ...
    loadWidget = () => {
        if (this.props.config === undefined)
            return;
        app.get(`/services/openweather/next5daysforecast/${this.props.config.data.city.toLowerCase()}`).then((response) => {
    ...
    Item = (props) => {
        ...
        return (
            <Paper variant="outlined" sx={{ maxHeight: 300, mx: 2 }}>
                ...
                <Typography variant="h6" gutterBottom component="div">{day} {monthNames[month - 1]} {year} {hour}:{(date.getMinutes()<10?'0':'')}{minutes}</Typography>
